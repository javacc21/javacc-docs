...
One more kind of Token to mention is the Private Regular Expression, which is any Token whose name begins with a "#". These Tokens are used as meaningful "abbreviations" used in the definitions of other Tokens. For example, imagine your system uses 5 numbers for User IDs. Instead of entering <USER_ID : ["0"-"9"] ["0"-"9"] ["0"-"9"] ["0"-"9"] ["0"-"9"]>, you could create a private RegEx and then use that when defining other Tokens. For example:
    TOKEN: <USER_ID: <NBR> <NBR> <NBR> <NBR> <NBR> >
         | <#NBR: ["0"-"9"] > ;

We'll cover ranges in the next section, but in this example the ["0"-"9"] means any single digit between 0 and 9 can be a <NBR>. 

Another good use for private RegExs is to emphasize certain items that might otherwise be glossed over. For example, imagine in the above USER_ID, the fourth digit was in the range 0-5 instead of 0-9. It would be way too easy to miss this little detail buried way down in the Token's definition if you specified each range separately. However, if you made a private RegEx just for that digit, say LO_NBR, that digit would stand out from the others; e.g., <NBR> <NBR> <NBR> <LO_NBR> <NBR>.
