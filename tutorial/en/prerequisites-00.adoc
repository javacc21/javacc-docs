...
This command assumes that the javacc-full.jar is in your current directory. If it is installed in a different directory, include the path, such as `~/java/libs/javacc-full.jar'.  Now you should be able to type `jcc` to invoke JavaCC. Well, try it:
...
====
*Tangent:* For most Linux distributions, the _good enough_ text editor I refer to above is `nano`, not `vim`. `Vim` is usually installed on most Unix and Linux distributions, but unless you are already familiar with vim` (or its predecessor `vi`), it would be masochistic to learn to use it!
====
