...
This command assumes that the javacc-full.jar is in your current directory. If it is installed in a different directory, include the path, such as `~/java/libs/javacc-full.jar`. It is also important that you include the quotes around everything after the equals sign.
